...
namespace SoftOmni.SwiftRd.Language.Base.Implementations.Constrained.LeafNodes;

public abstract class LeafNode<TFamily, TSelf, TInternalNode> : LeafElementBase,
    ILeafNode<TFamily, TInternalNode, TSelf>
    ...
    where TInternalNode : class, IInternalNode<TFamily, TInternalNode, TSelf>
{
    ...
    protected LeafNode(IEditableBuffer underlyingBuffer, TInternalNode parentNode, int parentIndex,
        int parentTextIndex)
    {
        UnderlyingBuffer = underlyingBuffer;
    ...
    public void AttachToParent(IInternalNode newParent, int parentIndex)
    {
        if (!newParent.GetType()
                .IsAssignableFrom(typeof(TInternalNode))) //TODO: Review and adjust documentation
        {
